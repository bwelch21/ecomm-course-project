
<!DOCTYPE html>
<!--[if lt IE 7 ]><html class="ie ie6" lang="en"> <![endif]-->
<!--[if IE 7 ]><html class="ie ie7" lang="en"> <![endif]-->
<!--[if IE 8 ]><html class="ie ie8" lang="en"> <![endif]-->
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en"> <!--<![endif]-->
<head>
  <meta charset="utf-8">
  <title>About Us</title>
  <meta name="description" content="">
  <meta name="author" content="">
</head>

<body>



  <!-- Primary Page Layout
    ================================================== -->

    <?php include("header.php"); ?>

    <div class="container" >




      <article class="twelve columns main-content">
        <h1>Hoos Home Cooking?</h1>
        

        
        <p> It is our mission to provide a small slice of comfort during the school year, in what can be a tumultuous experience, while also reducing food waste. With busy schedules and heavy work loads hot, home cooked meal can be hard to come by. </p>

        <p>Hoo’s Home Cooking shared marketplace allows you to be a cook, consumer, or even both! If you are preparing a meal and are anticipating multiple portions of leftovers you can easily avoid food waste by placing a notification on our marketplace. With our easy to use interface cooks can easily specify ingredients, flag major food allergens, specify price, indicate price and set a window for pickup time. All customers have to do is <strong>pay, pickup and enjoy </strong>their meal. </p>

        <div class="container">

          <div class="ten columns">

            <div class="flex-container">
              <div class="flexslider">
                <ul class="slides">

                  <li><img src="images/food_trade.jpg" alt="community" align="middle"/> </li> 

                </ul>
              </div>
            </div>
          </div>
        </div>

        <p>We hope to foster a <strong>community</strong> platform where students and faculty and share portions of their home cooked meal for a profit. A valid virginia.edu email is required for sign up, we hope our home cooked community reflects the integrity of the UVA community. </p>

        <div class="container">

          <div class="ten columns">

            <div class="flex-container">
              <div class="flexslider">
                <ul class="slides">

                  <li><img src="images/uva_community.jpg" alt="community" align="middle"/> </li> 

                </ul>
              </div>
            </div>
          </div>
        </div>

        <p>Food waste is a huge issue and Americans are global leaders in the production of food waste. The Hoos Cooking company helps to <strong>reduce food waste</strong> by allowing students to sell their leftovers</p> 
        <div class="container">

          <div class="ten columns">

            <div class="flex-container">
              <div class="flexslider">
                <ul class="slides">
                  <li>

                    <img src="images/food_waste.png" alt="food waste" align="middle"/> </li>

                  </ul>
                </div>
              </div>
            </div>
          </div>          
        </article>

        <!-- End main Content -->
        
        
        <aside class="four columns right-sidebar">



          <div class="sidebar-widget">
            <h2>Testimonials</h2>
            <blockquote class="testimonial no-after">
             My go to when I'm feeling stressed! Nothing like a homecooked meal.
             <cite>A Stressed Student</cite>
           </blockquote>

           <blockquote class="testimonial no-after">
             Easy way to make extra pocket change AND eliminate food waste!
             <cite>An Enviornmental Entrepraneur</cite>
           </blockquote>

           <blockquote class="testimonial no-after">
            Suddenly every student has the possibility to be a restaurante owner from the comfort of their own home!
            <cite>Small Business Owner</cite>
           </blockquote>

           <blockquote class="testimonial no-after">
            Infiniteley better than dining hall food!
            <cite>Previously Disgruntled First Year</cite>
           </blockquote>

           <blockquote class="testimonial no-after">
            Hoos Cooking is the best student-run business I've seen in my entire time here!
            <cite>Seasoned Fourth Year</cite>
           </blockquote>


         </aside>
         <!-- End Right Sidebar -->

       </div>

       <?php include("footer.html"); ?>

       <script src="js/jquery.prettyPhoto.js"></script>
     </body>
     </html>

